...
const CabinetButton: React.FC<CabinetButtonProps> = ({
  ...
  >
    ...
    <span className="pointer-events-none absolute inset-0 z-0 rounded-[1.65rem] border border-black/10" />
    <span className="pointer-events-none absolute inset-[6px] z-0 rounded-[1.5rem] border border-white/16 sm:inset-[7px] sm:rounded-[1.65rem]" />
    <div className="relative z-10 flex h-full w-full items-center gap-4">
      <span
        className={`relative flex h-[4.45rem] w-[4.45rem] shrink-0 items-center justify-center overflow-hidden rounded-[1.45rem] border-[2.5px] backdrop-blur-md shadow-[inset_0_1px_0_rgba(255,255,255,0.78),0_14px_24px_rgba(0,0,0,0.14)] sm:h-[4.9rem] sm:w-[4.9rem] sm:rounded-[1.6rem] ${cabinetToneClassName[tone].icon}`}
      >
        <span className="pointer-events-none absolute inset-x-2 top-1.5 h-4 rounded-full bg-white/28 blur-sm" />
        {icon}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block truncate font-serif text-[1.7rem] font-black leading-none sm:text-[1.95rem]">{label}</span>
        <span className="mt-2 block truncate text-[0.7rem] font-black uppercase tracking-[0.18em] text-current/78 sm:text-[0.74rem]">
          {detail}
        </span>
      </span>
      {disabled ? (
        <span
          className={`absolute right-3 top-3 rounded-full px-2.5 py-1 text-[0.58rem] font-black uppercase tracking-[0.22em] ${cabinetToneClassName[tone].badge}`}
        >
          Em Breve
        </span>
      ) : null}
...
export const Menu: React.FC<MenuProps> = ({ onSelectMode, onOpenCollection, profile, onEditProfile }) => {
  ...
  return (
    <motion.div
      ...
      className="relative flex h-full w-full items-center justify-center overflow-y-auto bg-[#ece3d3] p-3 text-[#31271e] no-scrollbar sm:p-5"
    >
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 rounded-full border border-[#b8ab93] bg-white/70 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-[#6b5a47] shadow-[0_10px_18px_rgba(0,0,0,0.06)]">
        Build {buildLabel}
      </div>
      ...
      <div className="relative z-10 w-full max-w-[68rem]">
        ...
        <div className="paper-panel relative z-10 overflow-hidden rounded-[2rem] border-[4px] border-[#4b3527]/25 px-4 py-5 shadow-[0_35px_80px_rgba(0,0,0,0.16)] sm:px-7 sm:py-7">
          <div className="absolute inset-0 bg-white/28" />
          <div className="absolute inset-y-[14px] left-[38px] right-[14px] rounded-[1.6rem] border border-[#d9c8a9]" />
          <div className="pointer-events-none absolute bottom-[18px] left-[34px] right-[22px] top-[18px] rounded-[1.45rem] border border-white/32" />

          <div className="relative flex flex-col gap-6">
            <div className="flex flex-col items-center text-center">
              <div className="flex w-full items-center justify-between gap-4 pl-[2.35rem] pr-4 sm:pl-[2.55rem] sm:pr-6">
                <div className="flex items-center gap-4 text-left">
                  <span className="flex h-[5.2rem] w-[5.2rem] items-center justify-center rounded-[1.8rem] border-2 border-[#efcf78] bg-[#f7eac5] text-[2.6rem] shadow-[0_14px_24px_rgba(0,0,0,0.08)] transition-transform group-hover:-translate-y-0.5 sm:h-[5.8rem] sm:w-[5.8rem] sm:text-[3rem]">
                    {profile.avatar}
                  </span>
                  <div className="min-w-0">
                    <span className="block text-[0.68rem] font-black uppercase tracking-[0.34em] text-[#a96e43]">
                      Duelista
                    </span>
                    <span className="mt-1 block font-serif text-[1.55rem] font-black uppercase leading-none text-[#5b2408] sm:text-[1.8rem]">
                      {displayName}
                    </span>
                  </div>
                </div>
                ...
                >
                  ...
                  <span className="pointer-events-none absolute inset-[4px] z-0 rounded-[1.1rem] border border-white/18 sm:rounded-[1.25rem]" />
                  ...
                  >
                    ...
                    <h1 className="relative font-serif text-[3.3rem] font-black uppercase leading-[0.86] text-amber-950 drop-shadow-[0_2px_0_rgba(255,248,220,0.45)] [text-shadow:0_0_18px_rgba(238,196,94,0.18),0_4px_14px_rgba(214,165,63,0.16)] sm:text-[4.8rem] lg:text-[5.7rem]">
                      SYLLABLE
                      <br />
                      BATTLE
                    </h1>
                    <motion.div
                      ...
                      className="relative mt-3 h-1.5 w-32 origin-center rounded-full bg-[#e2a438] shadow-[0_0_18px_rgba(226,164,56,0.28)]"
                    />
                  </motion.div>
              </div>

            <div className="ml-[18px] rounded-[1.9rem] border border-[#d8c9b0] bg-white/36 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] sm:ml-[20px] sm:p-5">
